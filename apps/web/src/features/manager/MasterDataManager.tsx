  ...
  const handleCreateDoctor = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast({
        ...
        message: `${docName} submitted for manager approval`,
      });
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast({
        ...
        message: `${prodName} added to product catalog`,
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Field Data</h1>
          <p className="text-xs text-slate-500">
            Maintain customer directories, products, and representative territories
          </p>
        </div>
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-slate-100 text-slate-400 font-semibold">
                  ...
                  <th className="pb-3">Customer Type</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {doctors.map((doc: any) => (
                  <tr key={doc.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3 text-emerald-800 font-semibold">Doctor</td>
              ...
              <tbody className="divide-y divide-slate-100">
                {products.map((prod: any) => (
                  <tr key={prod.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3 text-slate-700 font-medium">{prod.category || 'General'}</td>
      ...
      <Modal
        ...
        description="Add a doctor to the field directory for manager approval"
        maxWidth="lg"
      >
        <form onSubmit={handleCreateDoctor} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Clinic / Center</label>
              <input
                type="text"
                required
                value={docHospital}
